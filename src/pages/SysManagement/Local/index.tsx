...
import api from "../../../api";
import {
  localNonPermissionRegistration,
  localPermissionRegistration,
  useLocalList,
  useNonPermissionLocalList,
  usePermissionLocalList,
} from "../../../api/localManagement";
import {
  LocalListType,
  LocalNonPermissionRegistrationType,
  LocalNonPermissionType,
  LocalPermissionListType,
  LocalPermissionRegistrationType,
...
export default function LocalManagement() {
  ...
  //현장 미허가 솔루션
  const {
    data: localNonPermissionData,
    refetch: refetchLocalNonPermissionData,
  } = useNonPermissionLocalList(selectUserNo);
  const [localNonPermission, setLocalNonPermission] = useState<
    LocalNonPermissionType[]
  >([]);
        ...
        <Stack width={"100%"} height={"95%"} gap={1}>
          ...
          <TableBox width={"100%"} height={"50%"} gap={1}>
            <Stack gap={1} width={"64%"} height={"100%"}>
              ...
              <GrayBox>
                <BasicButton onClick={() => refetchLocalPermissionData()}>
                  새로고침
                </BasicButton>
              </GrayBox>
            </Stack>
            ...
            <Stack width={"34%"} height={"100%"} gap={1}>
              ...
              <TableBox.Inner>
                ...
                >
                  ...
                  <CheckboxTable.Tbody>
                    {(localNonPermissionData?.data.contents || []).map(
                      (item) => (
                        <CheckboxTable.Tr key={item.slutnId} id={item.slutnId}>
                          <CheckboxTable.CheckboxTd
                            item={item}
              ...
              <GrayBox>
                <BasicButton onClick={() => refetchLocalNonPermissionData()}>
                  새로고침
                </BasicButton>
              </GrayBox>
            </Stack>
