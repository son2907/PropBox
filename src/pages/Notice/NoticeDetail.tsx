...
export default function NoticeDetail() {
  ...
  useEffect(() => {
    if (noticeDetail) {
      console.log("noticeDetail.noticeSj:", noticeDetail.noticeSj);
    } else {
      console.log("noticeDetail 비어있습니다.");
    }
  }, [noticeDetail]);
    ...
    <>
      <Stack bgcolor={"white"} width={"100%"} height={"100%"}>
        <Stack direction={"row"} padding={1} width={"100%"} height={"5%"} gap={2} alignItems={"center"} marginTop={1}>
          ...
          <Box>
            <BasicInput sx={{ width: "650px" }} value={noticeDetail ? noticeDetail.noticeSj : ""} readOnly></BasicInput>
          </Box>
        </Stack>
        ...
        <Stack justifyContent={"space-between"} width={"100%"} direction={"row"} alignItems={"center"}>
          <Stack direction={"row"} padding={1} alignItems={"center"} width={"80%"} gap={1}>
            <BasicButton onClick={handleHideForADay}>하루동안 보이지않기</BasicButton>
          </Stack>
